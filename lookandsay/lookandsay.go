...
type LookAndSayChannels struct {
	StopChan  chan bool
	WriteChan chan string
}

func LookAndSayWriter(writer func(int) string) LookAndSayChannels {
	channels := LookAndSayChannels{
		make(chan bool),
		make(chan string),
	}
	...
	go func(
		...
		for {
			...
			case <-channels.StopChan:
				return
			case channels.WriteChan <- current:
...
func nextLookAndSay(writer func(int) string, current string) string {
	...
	for chunk, remainder := splitCharRange(current); chunk != ""; chunk, remainder = splitCharRange(remainder) {
